...
// Напишите регулярку, которая сделает из этой строки следующую:'#abc# #def# #xyz#';
// let str = 'abc def xyz';
// console.log(str.replace(/\b/g, "#"))

// Напишите регулярку, которая сделает из этой строки следующую:'a+b+c d+e+f x+y+z';
// let str = 'abc def xyz';
// console.log(str.replace(/\B/g, "+"))
...
// В данном уроке мы рассмотрим метод test, который проверяет, есть ли в строке хотя бы одно совпадение с регуляркой.
...
// Определите, начинается ли переданная строка с 'https://'.
// let str = "https://code.mu/ru/javascript/book/supreme/regular/method-test/";
// let reg = /https:\/\//g
...
// Карманы представляют собой способ разбить найденное на отдельные части.
// Для их использования нужно заключить часть регулярки в круглые скобки.
// В этом случае в массиве с результатом кроме найденной строки также появится и то, что попало под регулярку, стоящую в этих скобках.

// Положите день в первый карман, месяц - во второй, а год - в третий.
// let str = '31.12.2025';
// let result = str.match(/(\d{2})\.(\d{2})\.(\d{2})/)
// console.log(result);
...
// let sum = result.reduce((acc, item) => acc += (item * 1), 0)
// console.log(sum);
